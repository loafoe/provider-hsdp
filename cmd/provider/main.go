...
import (
	...
	"github.com/loafoe/provider-hsdp/internal/version"
)

func main() {
	var (
		app            = kingpin.New(filepath.Base(os.Args[0]), "HSDP support for Crossplane.").DefaultEnvars()
	...
	if *enableChangeLogs {
		...
		clo := controller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-hsdp:%s", version.Version))),
		}
