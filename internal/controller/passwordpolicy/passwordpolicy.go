...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	if err != nil {
		...
		// GetPasswordPolicyByID does its own client-side check that the
		// returned policy's ID matches, and synthesizes ErrNotFound (not a
		// 404) when it doesn't - handle that as not-found too.
